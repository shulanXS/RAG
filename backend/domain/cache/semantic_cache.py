...
class RedisSemanticCache(SemanticCache if REDISVL_AVAILABLE else object):
    ...
    def __init__(
        ...
        """P3.1(plan §4.3): 加 tenant_id 强制隔离。

        实现:per-tenant 独立 HNSW index(suffix `_t{tenant_id}`)。
        避免 tenant A 的 query embedding 命中 tenant B 缓存(信息泄露)。
        """
