...
# =============================================================================
# P2-B3: 错误分类(保留) + 删 _RerankCache(P3.1)
# =============================================================================

# 瞬时错误: 网络 / 限流 / 超时 — 走 RRF fallback 路径
...
def is_transient_error(exc: BaseException) -> bool:
    ...
    用于在 rerank 失败时区分是否走 RRF fallback。
    """
...
# P3.1: 删除 _RerankCache / RerankCacheStats / get_default_rerank_cache。
# 原因:实测命中率 < 1%(top-50 候选每次由 retrieval 决定,chunk 列表内容
#       难以完全一致);rerank 80ms 在 1.2s E2E 中可忽略;
#       2 个全局变量违反"显式依赖优于隐式状态"。
# 见 plan §3.2。
...
class CohereReranker(CrossEncoderReranker):
    ...
    def rerank(
        ...
        P3.1: 删 LRU 缓存(见 plan §3.2)。Cohere API 有 batch 优化,
        一次请求多个 doc 比多次单 doc 调用更快。
        """
        if not chunks:
            return []
...
class BGEReranker(CrossEncoderReranker):
    ...
    def rerank(
        ...
        P3.1: 删 LRU 缓存(见 plan §3.2)。
        CrossEncoder 的输入是 [query, doc] 对列表,自动处理 batch。
        """
        if not chunks:
            return []
